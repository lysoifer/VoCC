...
voccTraj <- function(lonlat, vel, ang, mn, tyr, trajID = 1:nrow(lonlat), correct = FALSE){
  ...
  if(correct == TRUE){
    e <- ext(terra::extend(vel, 1)) # extend by 1 unit resolution in all directions
  ...
  while(sum(remaining <= 0) != nc){  # while there is at least one trajectory active
    ...
    lonlat[resto,] <- as.data.frame(destPoint(llold[resto,], ang[fcells[resto]][,1], dis)) # distance input in meters. The function adjusts internally for -180-180 and pole crossings
    ...
    if(is.list(Trajxy)){
      ...
      Trajcells = cellFromXY(vel, Trajxy)
    ...
    if(sum(is.na(velend)) > 0) {
      remaining[resto][is.na(velend)] <- remaining[resto][is.na(velend)]-((distGeo(llold[resto,][is.na(velend),], oldxy[is.na(velend),])/1000)/abs(vel[fcells[resto]][,1][is.na(velend)]))
    }
    if(sum(!is.na(velend))>0) {
    ...
    # For those ending in land cells update lonlat info to the last marine point (i.e., non-NA point)
      ...
      if(sum(!empty) > 0){
        ...
        for(k in 1:length(leftcell)){
          leftcell[k] <- ifelse(vel[focal[k]][,1]>0 & sum(sst[[k]] < mn[focal[k]][,1]) > 0, leftcell[[k]][which.min(sst[[k]])],
                                ifelse(vel[focal[k]][,1]<0 & sum(sst[[k]] > mn[focal[k]][,1]) > 0, leftcell[[k]][which.max(sst[[k]])], NA))
        }
